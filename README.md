...
<title>StyleCart — Working Demo</title>
<style>
  /* simple reset & system font */
  *{box-sizing:border-box;margin:0;padding:0}
  html,body{height:100%}
  body{
    font-family: system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial;
    background:#f5f7fb;color:#162029;
  ...
  /* header */
  header{background:#fff;box-shadow:0 6px 20px rgba(19,35,47,.06);position:sticky;top:0;z-index:50}
  .header-inner{display:flex;align-items:center;justify-content:space-between;max-width:1200px;margin:0 auto;padding:14px 20px;gap:12px}
  .brand{display:flex;align-items:center;gap:10px;font-weight:700;color:#3b69f5}
  .brand svg{width:22px;height:22px;fill:#ff6b6b}
  nav ul{list-style:none;display:flex;gap:18px;align-items:center}
  nav a{text-decoration:none;color:#132028;font-weight:500;font-size:14px}
  .actions{display:flex;align-items:center;gap:12px}
  .search{display:flex;align-items:center;background:#f1f4f8;padding:8px 12px;border-radius:999px}
  .search input{border:0;background:transparent;outline:none;width:180px;font-size:14px}

  .icon-btn{background:transparent;border:0;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;padding:8px;border-radius:8px}
  .cart-count{position:relative;display:inline-block;margin-left:6px;font-weight:700;color:#fff;background:#ff6b6b;border-radius:12px;padding:2px 7px;font-size:12px}

  /* hero */
  .hero{background:linear-gradient(120deg,#4a6de5,#8a5cff);color:#fff;border-radius:10px;padding:36px;margin:20px 0;display:flex;gap:28px;align-items:center}
  .hero-left{max-width:640px}
  .hero h1{font-size:34px;line-height:1.05;margin-bottom:12px}
  .hero p{opacity:.95;margin-bottom:18px}
  .hero img{width:440px;height:280px;object-fit:cover;border-radius:8px;box-shadow:0 10px 30px rgba(11,31,88,.12)}

  /* categories */
  .categories{display:flex;gap:10px;justify-content:center;margin:26px 0}
  .category-btn{padding:10px 18px;border-radius:999px;border:1px solid #e6eefc;background:#fff;cursor:pointer;font-weight:600}
  .category-btn.active{background:linear-gradient(120deg,#4a6de5,#8a5cff);color:#fff;border:0;box-shadow:0 8px 24px rgba(74,109,229,.12)}

  /* product grid */
  .product-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:22px}
  .card{background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 8px 24px rgba(14,30,65,.06);display:flex;flex-direction:column}
  .card .img{height:200px;overflow:hidden}
  .card img{width:100%;height:100%;object-fit:cover;display:block}
  .card .info{padding:14px;display:flex;flex-direction:column;gap:8px;flex:1}
  .title{font-weight:700;font-size:16px}
  .price{color:#3b69f5;font-weight:800}
  .actions-row{margin-top:auto;display:flex;gap:10px;align-items:center}
  .add-btn{flex:1;padding:10px;border-radius:8px;border:0;background:#3b69f5;color:#fff;cursor:pointer;font-weight:700}
  .wish-btn{width:44px;height:44px;border-radius:8px;border:0;background:#f1f4f8;display:inline-flex;align-items:center;justify-content:center;cursor:pointer}
...
<main class="container">
  ...
  <section class="product-grid" aria-live="polite">
    ...
    <div class="card product-card" data-cat="women" data-price="39.99">
